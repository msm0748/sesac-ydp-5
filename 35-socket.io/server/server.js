...
io.on('connection', (socket) => {
  ...
  socket.on('setNick', (nick) => {
    ...
    if (Object.values(nickObjs).indexOf(nick) > -1) {
      ...
      io.emit('notice', { nick: 'system', msg: `${nick} 님이 입장하셨습니다.` }); // 전체 공지
  ...
  socket.on('disconnect', () => {
    ...
    io.emit('notice', { nick: 'system', msg: `${nickObjs[socket.id]} 님이 퇴장하셨습니다.` });
    delete nickObjs[socket.id];
    updateList();
  });

  socket.on('send', (data) => {
    io.emit('newMessage', { nick: nickObjs[socket.id], msg: data.msg });
  });
